...
## Initial Steps
Identifying the root cause(s) of performance drops is a necessary step before optimizing Java and Minecraft's in-game settings. Beyond hardware considerations, other factors that may contribute to performance issues in Minecraft include applications that consume a significant amount of resources such as processing time or system memory.

!!! tip "Performance Tweaks & Optimization"

	While the techniques outlined in this article offer optimization strategies for Minecraft, actual performance gains depend on individual system specifications. Improvements may range from substantial to negligible.
...
!!! tip "Java Installation"
	...
	The official Minecraft launcher already comes with Java built in, so you don’t need to install it separately. You only need a manual Java install if you use third-party launchers like [Prism](https://prismlauncher.org/) or run different Minecraft versions that require different Java versions.
