...
You can connect to the server through either the `telzenith.xyz` or `xeriscape.network` domain. For detailed instructions on [connecting to the server](./welcome-guide/connecting) and beginning your adventure, please refer to the [Welcome Guide](./welcome-guide/).

If you're experiencing difficulties connecting to the server, refer to our [Connection Troubleshooting article](./troubleshooting/connection). This resource provides several potential solutions designed to assist you in resolving the problem and successfully connecting to the Minecraft server.

### What Minecraft version?
You must be running an officially licensed copy of Minecraft to connect to the server as players using pirated, or "cracked", launchers (such as TLauncher) will be blocked from joining. Furthermore, your client must be running a version of Minecraft that is equal to, or newer than, the version the server is currently running.

You can check the current required version in the Discord server [status feed (in the channel topic header)](https://discord.com/channels/555868544244514836/1306843733060354058), or by attempting to connect, as outdated clients are automatically disconnected with an update notification. Please note that if your client version is newer than the server's version, you will only be able to access features and content present in the server's running version.

### Can I invite other people?
Since our server is public, you are more than welcome to invite any friends or other players you'd like to join you. Feel free to also share the server wherever you like.

### Can I use an alt account?
Alternate accounts (or 'alts') are permitted on the server, provided they are used in a manner that adheres strictly to fair play. This means that alternate accounts must not interact with each other and cannot be used to unfairly boost the primary player account in any way.

For detailed examples and a full clarification this policy, please refer to the [server guidelines](./guidelines). If you have additional questions after reviewing the guidelines, feel free to reach out directly to the staff team for assistance.
...
### Can I support the server?
Our server is available to everyone entirely free of charge and will never be monetized. You are welcome to bring anyone you wish to play with. We encourage you to share your positive experiences and promote the server, as simply playing on it and interacting with others is the best way to support us!

If you wish to contribute directly, boosting the Discord server is the recommended way. This helps us unlock features that Discord offers behind Server Boosts. As a thank you, you will automatically be granted the Booster role on both the Discord and Minecraft servers. Please note however, that we do not receive any monetary payout from Discord for these boosts.
