...
### Managing Warps
To create a warp, use the `/warp set <name>` command with the chosen warp name filling in the `<name>` argument. Players will need an initial $5,000 in order to proceed with the creation of a warp; Once created, will need to pay an additional $250 every seven real-life days as a rental fee in order to maintain that warp.
